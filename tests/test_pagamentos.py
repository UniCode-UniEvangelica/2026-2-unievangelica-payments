...
def test_validar_metodo_pagamento():
    ...
    # Arrange
    pagamentoValido = 'pix'
    pagamentoInvalido = 'cheque'

    # Act
    resultadoValido = validar_metodo_pagamento(pagamentoValido)
    resultadoInvalido = validar_metodo_pagamento(pagamentoInvalido)
    
    # Assert
    assert resultadoValido == True
    assert resultadoInvalido == False
...
def test_processar_reembolso():
    ...
    # Arrange
    valorPago = 100
    valorReembolsoValido = 50
    valorReembolsoInvalido = 150
    valorReembolsoLimite = 100

    # Act
    resultadoValido = processar_reembolso(valorPago, valorReembolsoValido)
    resultadoInvalido = processar_reembolso(valorPago, valorReembolsoInvalido)
    resultadoLimite = processar_reembolso(valorPago, valorReembolsoLimite)

    # Assert
    assert resultadoValido == 50
    assert resultadoInvalido == -1
    assert resultadoLimite == 0